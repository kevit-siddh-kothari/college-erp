...
const signUp = async (req, res) => {
    try {
        ...
        const user = new user_module_1.User({ username: req.body.username, password: req.body.password, role: req.body.role });
        await user.save();
...
const logIn = async (req, res) => {
    try {
        ...
        const user = await user_module_1.User.findOne({ username: req.body.username });
        if (!user) {
