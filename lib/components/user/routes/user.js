...
// Define routes
userRouter.post('/login', validator.body('username').notEmpty().withMessage(`username is required !`), validator.body('password').notEmpty().withMessage(`password is required !`), admin_controller_1.logIn);
userRouter.post('/signup', validator.body('password').isLength({ min: 7 }).withMessage(`password mus be of minimum 7 characters !`), validator.body('username').notEmpty().isEmail().withMessage(`please enter a valid email format !`), validator
    ...
    .notEmpty(), admin_controller_1.signUp);
