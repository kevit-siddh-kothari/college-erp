...
const addBatch = async (req, res) => {
    try {
        ...
        const departmentId = await department_1.Department.findOne({ departmentname: req.body.department });
        if (!departmentId) {
            throw new Error(`no department exists with name ${req.body.department}`);
        }
        const checkForExistingBatch = await batch_1.Batch.findOne({ year: req.body.year });
        if (checkForExistingBatch) {
            ...
            await batch_1.Batch.updateOne({ year: req.body.year }, { $push: { branches: newBranch } });
