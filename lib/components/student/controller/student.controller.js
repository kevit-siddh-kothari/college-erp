...
const batch_module_1 = require("../../batch/module/batch.module");
...
class StudentController {
    ...
    async addStudent(req, res) {
        try {
            ...
            const departmenId = await department_module_1.Department.findOne({ departmentname: req.body.departmentname }, { _id: 1 });
            if (!departmenId) {
            ...
            const batchData = await batch_module_1.Batch.find({ year: req.body.batch }, { branches: 1, _id: 0 }).lean();
            ...
            await student.save();
            await batch_module_1.Batch.updateOne({ 'year': req.body.batch, 'branches.departmentId': departmenId._id }, { $inc: { 'branches.$.availableSeats': -1, 'branches.$.occupiedSeats': 1 } });
    ...
    async deleteStudentById(req, res) {
        ...
        try {
            ...
            await batch_module_1.Batch.updateOne({ 'year': student.batch, 'branches.name': student.department }, { $inc: { 'branches.$.availableSeats': 1, 'branches.$.occupiedSeats': -1 } });
            await student_module_1.Student.deleteOne({ _id: student._id });
            await attendance_module_1.Attendance.deleteMany({ student: student._id });
